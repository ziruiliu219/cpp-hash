/// Quick profile: 4str_0int, ht=65536, 20 iterations. Matches Rust profile_taper.
#include <cstdio>
#include <cstdint>
#include <vector>
#include <string>
...
static inline int64_t HB(const uint8_t* d, size_t l, uint64_t s) { return static_cast<int64_t>(XXH3_64bits_withSeed(d,l,s)); }
...
int main() {
    ...
    for(size_t i=0;i<NK;i++){uint64_t h=0;for(size_t c=0;c<NS;c++)h=XXH3_64bits_withSeed(bs[c][i].data(),bs[c][i].size(),h);bh[i]=static_cast<int64_t>(h);}
    ...
    for(size_t i=0;i<nM;i++){uint64_t h=0;for(size_t c=0;c<NS;c++){auto s="miss_"+std::to_string(i)+"_"+std::to_string(c);std::vector<uint8_t> b(s.begin(),s.end());h=XXH3_64bits_withSeed(b.data(),b.size(),h);ps[c].push_back(std::move(b));}ph.push_back(static_cast<int64_t>(h));}
    ...
    for(int iter=0;iter<20;iter++){
        ...
        std::vector<taper::ColumnInput> cols; for(size_t c=0;c<NS;c++) cols.push_back(taper::ColumnInput::MakeVarchar(aptrs[c].data(),alens[c].data()));
